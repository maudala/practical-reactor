...
public class c13_Context extends ContextBase {
    ...
    public Mono<Message> messageHandler(String payload) {
        return Mono.deferContextual(contextView -> {
            var correlationId = contextView.<String>get(HTTP_CORRELATION_ID);
    ...
    public void pagination() {
        ...
        Flux<Integer> results = Mono.deferContextual(contextView -> {
                    var pageNum = contextView.<AtomicInteger>get("cur").getAndIncrement();
                    return getPage(pageNum);
                })
                .onErrorResume(throwable -> Mono.deferContextual(contextView -> {
                    var pageNum = contextView.<AtomicInteger>get("cur").get();
                    pageWithError.set(pageNum - 1);
                    return Mono.empty();
                }))
                .flatMapMany(Page::getResult)
                .repeat(10)
                .doOnNext(i -> System.out.println("Received: " + i))
                .contextWrite(Context.of("cur", new AtomicInteger()));
